...
class clasemanejo
{
	...
	{
		$objconexion = new claseconexion();
		$consulta = $objconexion->prepare("select p.idproducto, p.foto, p.nombreproducto, p.precio, p.stok, p.cantidad, p.color, p.marcap, p.categoriap, m.marca, c.categoria from productos as p inner join marcas as m on p.marcap=m.idmarca inner join categorias as c on p.categoriap=c.idcategoria where categoriap=:categoriap;");
